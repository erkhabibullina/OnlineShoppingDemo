...
import com.example.android.onlineshoppingdemo.security.PasswordHelpers;
...
public class DatabaseDriverAndroid extends SQLiteOpenHelper {
    ...
    protected Cursor getSales() {
        SQLiteDatabase sqLiteDatabase = getReadableDatabase();
        return sqLiteDatabase.rawQuery("SELECT * FROM SALES", null);
    }

    protected Cursor getSaleById(int saleId) {
        SQLiteDatabase sqLiteDatabase = getReadableDatabase();
        return sqLiteDatabase.rawQuery("SELECT * FROM SALES WHERE ID = ?;",
                new String[]{String.valueOf(saleId)});
    }

    protected Cursor getSalesToUser(int userId) {
        SQLiteDatabase sqLiteDatabase = getReadableDatabase();
        return sqLiteDatabase.rawQuery("SELECT * FROM SALES WHERE USERID = ?;",
                new String[]{String.valueOf(userId)});
    }

    protected Cursor getItemizedSales() {
        SQLiteDatabase sqLiteDatabase = getReadableDatabase();
        return sqLiteDatabase.rawQuery("SELECT * FROM ITEMIZEDSALES;", null);
    }

    protected Cursor getItemizedSaleById(int saleId) {
        SQLiteDatabase sqLiteDatabase = getReadableDatabase();
        return sqLiteDatabase.rawQuery("SELECT * FROM ITEMIZEDSALES WHERE SALEID = ?;",
                new String[]{String.valueOf(saleId)});
    }

    protected Cursor getUserAccounts(int userId) {
        SQLiteDatabase sqLiteDatabase = getReadableDatabase();
        return sqLiteDatabase.rawQuery("SELECT ID FROM ACCOUNT WHERE USERID = ?;",
                new String[]{String.valueOf(userId)});
    }

    protected Cursor getAccountDetails(int accountId) {
        SQLiteDatabase sqLiteDatabase = getReadableDatabase();
        return sqLiteDatabase.rawQuery("SELECT * FROM ACCOUNTSUMMARY WHERE ACCTID = ?;",
                new String[]{String.valueOf(accountId)});
    }

    protected Cursor getUserActiveAccounts(int userId) {
        SQLiteDatabase sqLiteDatabase = getReadableDatabase();
        return sqLiteDatabase.rawQuery("SELECT * FROM ACCOUNT WHERE userId = ? and ACTIVE = ?",
